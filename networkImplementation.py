...
from layers import FCLayer, QuadraticLayer, ActivationLayer, MatrixLayer
...
for j in range(AGES):
    ...
    net.add(MatrixLayer((28, 28), 10))
    net.add(ActivationLayer(sigmoid, sigmoid_prime))

    # train on 1000 samples
    # as we didn't implemented mini-batch GD, training will be pretty slow if we update at each iteration on 60000 samples...
    net.set_loss(mse, mse_prime)
    (errorArray, tensorArray, tensorw, delta) = net.fit(x_train[0:1000], y_train[0:1000], epochs=50, learning_rate=0.5)
    ...
    with open('Mepochs.csv', 'a', newline='') as file:
